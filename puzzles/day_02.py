...
class SubmarinePosition(TypedDict):
    horizontal: int
    depth: int


class SubmarineAimingPosition(TypedDict):
    horizontal: int
    depth: int
    aim: int
...
def aiming_position_reducer(
    position: SubmarineAimingPosition,
    command: tuple[str, int],
) -> SubmarineAimingPosition:
    match command:
        case ["forward", value]:
            position["horizontal"] += value
...
def day_2_part_1() -> int:
    ...
    """
    input_seq = get_input_seq(day=2).map(parse_command)
    initial_position = SubmarinePosition(horizontal=0, depth=0)

    # Use a reducer to sequentially apply position changes to an initial
    # position
    position = input_seq.reduce(position_reducer, initial_position)
    result = position["horizontal"] * position["depth"]
    print(f"Result for day 2, part 1: {result}")
    assert result == 2073315
    return result
...
def day_2_part_2() -> int:
    ...
    Now, the above example does something different:
        ...
        your aim is 0, your depth does not change. down 5 adds 5 to your aim,
        resulting in a value of 5. forward 8 adds 8 to your horizontal
        position, a total of 13. Because your aim is 5, your depth increases
        by 8*5=40. up 3 decreases your aim by 3, resulting in a value of 2.
        down 8 adds 8 to your aim, resulting in a value of 10. forward 2 adds
        2 to your horizontal position, a total of 15. Because your aim is 10,
    ...
    """
    input_seq = get_input_seq(day=2).map(parse_command)
    initial_position = SubmarineAimingPosition(horizontal=0, depth=0, aim=0)
    position = input_seq.reduce(aiming_position_reducer, initial_position)
    result = position["horizontal"] * position["depth"]
    print(f"Result for day 2, part 2: {result}")
    assert result == 1840311528
    return result
